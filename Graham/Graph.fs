...
(* TODO :   Implement a sparse, directed bipartite graph type. This'll provide a cleaner, safer
            way to represent relations where the source vertices are of one type and the target
            vertices are of another. *)
// TEMP : This will suffice as a bipartite graph representation for now, though it doesn't enforce
// the invariant that all edges need to flow from a vertex in the 'source' set to a vertex in the 'target' set.
/// A vertex-labeled sparse bipartite digraph.
type VertexLabeledSparseBipartiteDigraph<'SourceVertex, 'TargetVertex
    when 'SourceVertex : comparison
    and 'TargetVertex : comparison> =
    VertexLabeledSparseDigraph<Choice<'SourceVertex, 'TargetVertex>>
...
module VertexLabeledSparseBipartiteDigraph =
    ...
    let empty<'SourceVertex, 'TargetVertex when 'SourceVertex : comparison and 'TargetVertex : comparison> =
        VertexLabeledSparseBipartiteDigraph<'SourceVertex, 'TargetVertex>.Empty

    /// Determines if the graph is empty -- i.e., if it's vertex set is empty.
    let inline isEmpty (graph : VertexLabeledSparseBipartiteDigraph<'SourceVertex, 'TargetVertex>) =
        graph.IsEmpty
    ...
    //
    let inline addSourceVertex (vertex : 'SourceVertex) (graph : VertexLabeledSparseBipartiteDigraph<'SourceVertex, 'TargetVertex>) =
        graph.AddVertex (Choice1Of2 vertex)

    //
    let inline addTargetVertex (vertex : 'TargetVertex) (graph : VertexLabeledSparseBipartiteDigraph<'SourceVertex, 'TargetVertex>) =
        graph.AddVertex (Choice2Of2 vertex)

    //
    let inline addEdge source target (graph : VertexLabeledSparseBipartiteDigraph<'SourceVertex, 'TargetVertex>) =
        graph.AddEdge (Choice1Of2 source, Choice2Of2 target)

    //
    let inline addEdgeAndVertices source target (graph : VertexLabeledSparseBipartiteDigraph<'SourceVertex, 'TargetVertex>) =
        graph.AddEdgeAndVertices (Choice1Of2 source, Choice2Of2 target)
