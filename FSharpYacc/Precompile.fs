...
module Prepare =
    ...
    let private precompileImpl (spec : Specification) (options : CompilationOptions) =
        state {
    ...
    let precompile (spec : Specification) (options : CompilationOptions)
        ...
        // Create and execute a workflow via the precompileImpl function.
        PrecompilationState.empty
        |> State.execute (precompileImpl spec options)

    /// Creates a PrecedenceSettings record from a ProcessedSpecification.
    let precedenceSettings (taggedGrammar : TaggedAugmentedGrammar<_,_>)
