...
module private Unicode =
    ...
    let categoryCharSet =
        // Initialize the map so it contains an empty set for each Unicode category.
        // This serves two purposes:
        //  (a) To ensure the completed map contains an entry for each Unicode category,
        //      so we can just use Map.find instead of Map.tryFind.
        //  (b) It avoids a branch in the loop below.
        let mutable categoryChars =
            let unicodeCategories =
                Enum.GetValues typeof<UnicodeCategory> :?> UnicodeCategory[]

            (Map.empty, unicodeCategories)
            ||> Array.fold (fun categoryChars category ->
                Map.add category CharSet.empty categoryChars)
        ...
        // OPTIMIZE : If necessary, we could also use an array here instead of a Dictionary,
        // since the number of elements is small and static.
        for i = 0 to 65535 do
            /// The Unicode category of this character.
            let category = Char.GetUnicodeCategory (char i)

            // Add this character to the set for this category.
            categoryChars <-
                let charSet = Map.find category categoryChars
                Map.add category (CharSet.add (char i) charSet) categoryChars
