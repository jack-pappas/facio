    ...
    and [<Sealed>]
        ...
        member lexbuf.EndOfScan () : int =
            ...
            if bufferAcceptAction < 0 then
                let ex =
                    let msg = sprintf "Unrecognized input. (Line = %i, Column = %i)" endPos.Line endPos.Column
                    exn msg

                ex.Data.Add (
                    "Last Input Character",
                    lexbuf.Buffer.[lexbuf.BufferScanPos])
                ex.Data.Add (
                    "Lexeme",
                    lexbuf.Buffer.[lexbuf.BufferScanStart .. lexbuf.BufferScanStart + lexbuf.BufferScanLength])
